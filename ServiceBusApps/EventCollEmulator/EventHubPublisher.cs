...
namespace MGS.Base.EventHub
{
  public class EventHubPublisher
  {
    ...
    public EventHubPublisher(string eventHubUrl, string eventHubName, string senderKeyName, string senderKey, int poolSize = 5)
    {
      ...
      _msgFactories = new ConcurrentStack<MessagingFactory>();
    ...
    private void CreateEventHubPool()
    {
      ...
      {
        var fact = MessagingFactory.CreateFromConnectionString(_connectionString + ";TransportType=Amqp");
    ...
    public async Task<bool> PostMsg(byte[] msg, string partitionKey = null, IDictionary<string, object> sysProperties = null, IDictionary<string, object> properties = null)
    {
      ...
      try
      {
        ...
        if (sysProperties != null && sysProperties.Count > 0)
        {
          foreach (var kvp in sysProperties)
          {
            ed.SystemProperties.Add(kvp.Key, kvp.Value);
          }
        }
